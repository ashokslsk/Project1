...
import com.ashokslsk.popularmovies.provider.MovieProvider;
...
public class Utils {
    ...
    public static int toggleFavourite(Context context, Movie movie)
    {
        ...
        Uri.Builder uriBuilder = MovieProvider.CONTENT_URI.buildUpon();
    ...
    public static List<Movie> getFavouriteMovies(Context context)
    {
        ...
        if(movies.size()==0)
            Toast.makeText(context,context.getResources().getString(R.string.NoFavorite),Toast.LENGTH_LONG).show();
        return movies;
    }
